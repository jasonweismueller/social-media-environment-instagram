      ...
      {showComment && (
        <Modal onClose={() => setShowComment(false)} title="Write a comment">
          ...
          <div className="row-end">
            <button className="btn" onClick={() => { click("comment_cancel"); setShowComment(false); }}>Cancel</button>
...
function Feed({
  ...
  showInterventionLabelFor, showInterventionNoteFor, noteText, badgeTarget,
  log, showComposer
}) {
  return (
    <main className="container">
      ...
      {posts.map((p) => (
        <PostCard
          key={p.id} post={p} onAction={log} disabled={disabled} registerViewRef={registerViewRef}
          showInterventionLabel={showInterventionLabelFor === p.id}
          showInterventionNote={showInterventionNoteFor === p.id}
          noteText={noteText}
...
function AdminDashboard({
  ...
  interventionPostId, setInterventionPostId,
  interventionType, setInterventionType,
  noteText, setNoteText,
  badgeTarget, setBadgeTarget,
    ...
    <>
      <div className="nav">
        <div className="nav-inner">
          <IconLogo /><div className="brand">Fakebook — Admin</div>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: ".4rem", flexWrap: "wrap" }}>
            <Link to="/" className="btn">Open Feed</Link>
          </div>
        </div>
      </div>
      ...
      <div className="container">
        <div className="card" style={{ padding: "1rem" }}>
          ...
          <div style={{ display: "grid", gap: ".5rem", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))" }}>
            ...
            <label>Intervention post
              <select className="select" value={interventionPostId || "none"} onChange={(e) => setInterventionPostId(e.target.value === "none" ? null : e.target.value)}>
                <option value="none">None</option>
                {INITIAL_POSTS.map((p) => (<option key={p.id} value={p.id}>{p.id} — {p.author}</option>))}
              </select>
            </label>
            ...
            {interventionType === "note" && (
              <label style={{ gridColumn: "1 / -1" }}>Note text
                <input className="input" value={noteText} onChange={(e) => setNoteText(e.target.value)} />
              </label>
            )}
            <label>Verification badge
              <select className="select" value={badgeTarget} onChange={(e) => setBadgeTarget(e.target.value)}>
                <option value="none">None</option>
...
export default function App() {
  ...
  const [interventionPostId, setInterventionPostId] = useState(() => {
    ...
    const v = String(initialLabelParam).toLowerCase();
    return v === "none" || v === "0" ? null : v;
  });
  const itypeParam = (getUrlParam("itype") || "").toLowerCase();
  const [interventionType, setInterventionType] = useState(itypeParam === "note" ? "note" : itypeParam === "label" ? "label" : "none");
  const [noteText, setNoteText] = useState(getUrlParam("inote") ? decodeURIComponent(getUrlParam("inote")) : "There is no evidence that U.S. Immigration and Customs Enforcement (ICE) offers $750 for reporting people in the U.S. without authorization. The ICE tip form is meant for reporting crimes or suspicious activity, not for immigration enforcement rewards.");
  const badgeParam = (getUrlParam("badge") || "none").toLowerCase();
  const [badgeTarget, setBadgeTarget] = useState(["all", ...INITIAL_POSTS.map(p => p.id)].includes(badgeParam) ? badgeParam : "none");
  ...
  const showInterventionLabelFor = interventionType === "label" ? interventionPostId : null;
  const showInterventionNoteFor  = interventionType === "note"  ? interventionPostId : null;
        ...
        <Route
          ...
          element={
            <Feed
              ...
              disabled={disabled}
              showInterventionLabelFor={showInterventionLabelFor}
              showInterventionNoteFor={showInterventionNoteFor}
              noteText={noteText}
        ...
        <Route
          ...
          element={
            <AdminDashboard
              ...
              interventionPostId={interventionPostId} setInterventionPostId={setInterventionPostId}
              interventionType={interventionType} setInterventionType={setInterventionType}
              noteText={noteText} setNoteText={setNoteText}
              badgeTarget={badgeTarget} setBadgeTarget={setBadgeTarget}
              events={events}
